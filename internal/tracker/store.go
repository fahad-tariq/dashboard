...
func (s *Store) ReplaceAllWithAttribution(items []Item, attributionUserID int64) error {
	...
	// Upsert each item.
	stmt, err := tx.Prepare(`
		INSERT OR REPLACE INTO tracker_items (slug, title, type, category, priority, current_val, target_val, unit, done, graduated, added, completed, deadline, planned, from_idea, tags, images, list, user_id)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`)
	if err != nil {
		return err
	...
	for _, it := range items {
		...
		_, err := stmt.Exec(it.Slug, it.Title, string(it.Type), strings.Join(it.Tags, ","), it.Priority,
			it.Current, it.Target, it.Unit, it.Done, it.Graduated, it.Added, it.Completed, it.Deadline, it.Planned, it.FromIdea, string(tagsJSON),
